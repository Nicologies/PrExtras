package com.nicologies.prextras.common;

public class PrExtrasConstants {
    public final static String RunnerType = "PrExtras";
    public final static String DefaultExportParamName = "build.github_branch_name";
    public final static String TokenAuthType = "Token";
    public final static String UsernameAuthType = "Username";
    public final static String SystemWideTokenAuthType = "SystemWideToken";
    public final static String PrefixOfUserMapping = "user_mapping.";
}
